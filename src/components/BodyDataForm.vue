<template>
  <div class="member-information">
    ...
    <div class="detail-cell"
     ...
     :key="index">
      <div
        :class="[item.selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN' && item.showPicFlag === 1 ? 'none-title' : 'cell-title']"
      >
      ...
      <div
          class="cell-content"
          v-if="item.selectClassDictCode === 'D_FEATURE_CHOTYPE_DOWN' "
      ...
      <!-- 文本框 -->
      <div class="cell-content" v-if="item.inputClassDictCode === 'D_FEATURE_WRITETYPE_TEXT' ">
        <input
          placeholder="请填写相关信息"
          ...
          :type="item.fieldTypeDictCode === 'D_FEATURE_COLTYPE_DATE' ? 'tel' : 'text'"
          @focus="getFoucs"
      ...
      <div class="cell-content radius" v-if="item.selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN' &&  item.showPicFlag === 0">
        ...
        <van-checkbox-group v-model="item.valueDtoList" v-if="item.unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_MORE'">
          <van-checkbox
      ...
      <div class="cell-content model-div" v-if="item.selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN'  && item.showPicFlag === 1">
                <div >
                    <div class="mod-tel"><span class="red-star" v-if="item.mustFlag== true">*</span>
                    <span v-else>&nbsp;</span>{{item.featureName}}</div>
                    <ul>
...
export default {
  ...
  props: {
    ...
    routeName: String,
    sendAmount: String,
    sendMainFabricId: String,
    yuyueData:{},
    queryData:{},
    v_type: String,
    measureOpCode: String,
    cb: {
      type: Function
    }
  },
  ...
  methods: {
    ...
    async putUserInfo() {
      ...
      this.memberInfo.forEach((item) => {
        //  会员属性
        if (item.mustFlag == 1) {
          if(item.inputClassDictCode == 'D_FEATURE_WRITETYPE_SELECT' && item.selectClassDictCode == 'D_FEATURE_CHOTYPE_OPEN' && item.unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_MORE'){
            if(item.values.length == 0){
              Toast("必填项" + item.featureName + "未填，请填写");
              lock = false;
              return;
            }
          }else{
        ...
        if (res.mobilePhone !== null) {
          if (typeof this.cb === "function") {
            if (this.chichun == 1) {
              ...
              if(this.v_type == 'payCode'){
                this.s_lock = false
                let memberData = {
                  //  会员属性
                  customs: arr,
                  saleMeasureHdId: null
                };
                this.updateUser(data, memberData)
                return;
              }
              console.log("有走这里哦",this.v_type)
              saleMeasure = await this.cb();
              console.log("banwo",saleMeasure)
              // console.log("这个是、saleMeasure",saleMeasure)
              // let memberData = {
              //   //  会员属性
              //   customs: arr,
    ...
    updateUser(data, memberData) {
      if (memberData.saleMeasureHdId != undefined && memberData.saleMeasureHdId != null && memberData.saleMeasureHdId != '') {
        if(this.v_type == 'payCode'){
          console.log('走了预约+量体1',this.yuyueData);
          let b_data =this.queryData
          if(!!this.sendAmount){
            this.$set(b_data, "amount",this.sendAmount);
          }
          if(!!this.sendMainFabricId){
            this.$set(b_data, "mainFabricId",this.sendMainFabricId);
          }
          this.$router.replace({
                name:this.routeName,
                params: {
                   measureOpCode:this.submitData.measureOpCode,
                   liangti:this.submitData.liangti,
                   yuyue:this.yuyueData
                },
                query:b_data
              })
          // this.$emit('listenToChildEvent', memberData);
          return;
        }
        ...
        if(this.v_type == 'payCode'){
          ...
          this.submitData.measureOpCode = this.measureOpCode
          this.submitData.liangti = s_data
          console.log("有成功地址哦",this.successUrl)
          console.log("传之前的值",this.submitData)
          let b_data = this.queryData
          if(!!this.sendAmount){
            this.$set(b_data, "amount",this.sendAmount);
          }
          ...
          console.log('有successUrl',this.sendMainFabricId)
          this.$router.replace({
                name:this.routeName,
                params: {
                   measureOpCode:this.submitData.measureOpCode,
    ...
    getMemberInfo() {
      ...
      UserService.getAiMemberInfo(data).then((res) => {
        ...
        // res.forEach((item) => {
        //   console.log("有item哟",item)
        // });
        // for (let j = 0; j < _length; j++) {
          
        //   if (res[j].showFlag == 1) {
        //     this.$set(res[j], "values",'');
        //     _data.push(res[j])
        //   }
        // }
        if(res.length == 0){
            if(this.v_type == 'payCode'){
               var memberData = null
                ...
                console.log('有successUrl',this.routeName)
                this.$router.replace({
                      name:this.routeName,
                      params: {
                        measureOpCode:this.submitData.measureOpCode,
        ...
        console.log("memberInfo有值有",this.memberInfo)
      });
    },
    //  会员属性确认
    changeMember(value) {
