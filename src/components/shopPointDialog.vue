...
<template>
    <van-popup v-model="show" class="pain-dalog_point">
        <div class="successBox" >
            <img src="../assets/images/volume/redBag.png">
            <div class="li_word">
                <div class="word_style" v-if="custText">{{custText}}</div>
                <div class="word_style" v-else>送您&nbsp;&nbsp;<span class="word_num">{{msg}}</span>&nbsp;&nbsp;张抵用券</div>
            </div>
            <div class="li_go" >
                <div class="btn" @click="jumpMyReserve">
                    前往查看
                </div>
            </div>
            <div class="closeIcon" @click="closeDalog">
...
export default {
    ...
    methods: {
        closeDalog() {
            this.show = false
            console.log("this.showDalog",this.show)
        },
        jumpMyReserve() {
            if(this.routerUrl){
    ...
    created() {
        this.showDalog = this.show
        console.log("this.show11111",this.show)
        this.onInit();
    },
    mounted() {
