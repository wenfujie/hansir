...
export default {
    ...
    methods: {
        ...
        async addToWechatCardPkg() {
            this.cardInfo = await this.getVipCardInfo();
            this.$requestAll([this.getWxConfig(), this.getCardConfig(this.cardInfo.wxId)]).then((res) => {
                console.log("request", res)
                this.wxInit(res[0], this.cardInfo.wxId, res[1])
            })
        }
    },
    computed: {
        ...
        canAddToWxCardPkgs: function () {
            if (!this.cardInfo.wxId) return false;
            if (this.cardInfo.isSynctowx == 0) return false;
