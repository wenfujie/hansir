<template>
  <div class="design pain-style">
    ...
    <div v-if="show" class="submit" @click="postVolume1">提交预约</div>
    <!-- 省市区选择 -->
...
export default {
  ...
  created() {
    ...
    // this.getCityData();
    if (this.$route.params.fromPage == 'order' || this.$route.query.fromPage == 'order') {
      if (this.$route.params.billCode != null & this.$route.params.billCode != '' & this.$route.params.billCode != undefined) {

        this.billCode = this.$route.params.billCode
      }
      if (this.$route.query.billCode != null & this.$route.query.billCode != '' & this.$route.query.billCode != undefined) {
        this.billCode = this.$route.query.billCode
      }
      if (this.billCode !== null || this.billCode !== undefined || this.billCode !== '') {
        var j_data = {
          usrId: Storage.get('USER_INFO').usrId,
          ownCompanyId: Storage.get("COMPANYID").company_id,
          companyId: Storage.get("COMPANYID").company_id,
        ...
        Volume.getBillType(j_data).then(
          (res) => {
            this.rtlOrdHdCode = this.billCode
            var s_array = []
            for (var i = 0; i < res.length; i++) {
              // 新加代码
            ...
            this.saleOrdDtList = s_array
            Storage.set("rtlOrdHdCode", { "billCode": this.billCode });
            Storage.set("saleOrdDtList", this.saleOrdDtList);
          },
          () => {
            this.btnLock = false;
          }
    ...
    if (this.$route.params.fromPage == 'order') {
      // if (typeof this.$route.query.goodCodes == "string") {
      //   var arr = []
      //   arr.push(this.$route.query.goodCodes)
      //   this.partIds = arr
      // } else {
      //   this.partIds = this.$route.query.goodCodes
      // }
      this.partIds = this.$route.params.goodCodes.toString()
      var sdata = {
        usrId: Storage.get('USER_INFO').usrId,
        ownCompanyId: Storage.get("COMPANYID").company_id,
        companyId: Storage.get("COMPANYID").company_id,
        goodsCodes: this.$route.params.goodCodes.toString(),
        busContsCode: 'D_BUSCONTS_WSC'
      }
      Volume.getMeasureOpInfo(sdata).then(
        (res) => {
          if (res.measureOpCode == 'D_MSROPSIZE') {
            Storage.set("measureOpCode", { measureOpCode: 'D_MSROPSIZE' });
            this.show = false;
            this.toast = '填写身型数据，为您智能匹配尺码'
            this.$route.meta.title = '量体'
            document.title = '身型信息'
            this.otherBillCode = this.billCode
            this.sun_chichun = 1
            if (this.$route.params.type != null && this.$route.params.type != undefined && this.$route.params.type != '') {
    ...
    if (this.$route.query.fromPage == 'order') {
      // if (typeof this.$route.query.goodCodes == "string") {
      //   var arr = []
      //   arr.push(this.$route.query.goodCodes)
      //   this.partIds = arr
      ...
      // }
      this.partIds = this.$route.query.goodCodes
      var sun_data = this.$route.query.goodCodes.split(",")
      var sdata = {
        usrId: Storage.get('USER_INFO').usrId,
        ownCompanyId: Storage.get("COMPANYID").company_id,
        companyId: Storage.get("COMPANYID").company_id,
        goodsCodes: sun_data.toString(),
        busContsCode: 'D_BUSCONTS_WSC'
      }
      Volume.getMeasureOpInfo(sdata).then(
        (res) => {
          if (res.measureOpCode == 'D_MSROPSIZE') {
            ...
            this.sun_chichun = 1
            this.otherBillCode = this.billCode
            this.$router.meta.title = '量体'
            this.s_type = this.$route.query.type
            document.title = '身型信息'
          }
        },
        () => {
