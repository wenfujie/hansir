  ...
  <div class="member-information">
    ...
    <div class="detail-cell"
     ...
     v-for="(item, index)  in memberInfo" :key="index">
      <div
      :class="[item.selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN' && (item.inputClassDictCode=='D_FEATURE_WRITETYPE_SELECT' || item.showPicFlag === 1) ? 'none-title' : 'cell-title']"
      >
      ...
      <div
        class="cell-content"
        v-if="item.selectClassDictCode === 'D_FEATURE_CHOTYPE_DOWN' && item.inputClassDictCode
 == 'D_FEATURE_WRITETYPE_SELECT'"
        @click="showPopup(item)"
      >
        <input
          :placeholder="item.inputDesc"
          class="van-field__control"
          v-model="item.valuemName"
          readonly="readonly"
          unselectable="on"
          onfocus="this.blur()"
        >
      ...
      <!-- 没图单多选框 -->
      <div class="cell-content radius" v-if="item.selectClassDictCode == 'D_FEATURE_CHOTYPE_OPEN' &&  item.showPicFlag == 0">

        <div v-if="item.unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_ONE' || item.unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_MORE'">
          <div class="mod-tel">
                      <div class="mod_img">
                      <img src="../../assets/images/volume/feature.png"/>
                      </div>
                      <div class="displayinbl">
                     {{item.featureName}}
                     </div> 
                     <div class="red-star displayinbl" v-if="item.mustFlag== true">*</div>
                    <span v-else>&nbsp;</span>
          </div>
          <!--单选列表-->
      ...
      <!-- 带图选择框 begin -->
      <div class="cell-content model-div" v-if="item.selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN'  && item.showPicFlag === 1">
                <div >
                    <div class="mod-tel">
                      <div class="mod_img">
                      <img src="../../assets/images/volume/feature.png"/>
                    ...
                    <ul>
                          <li  class="mod-item" v-for="(item1, index1)  in item.valueDtoList" :key="index1" >
                              ...
                              <div v-if="item.unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_MORE'"  @click="muchToggleSelected(index,index1)">
                                <div class="type-img" :class="[item1.defaultFlag == 1 ? 'pain-red' : 'pain-white']">
                                    <div class="item-tip" v-if="item1.defaultFlag == 1"><img src="../../assets/images/common/choose.png" alt=""></div>
                                    <img :src="getImg(item1.fileUrl)" alt="">
                                </div>
...
//                str = str.replace(/\.\d\d$/,'')
        ...
        if (res.mobilePhone !== null) {
          ...
          for(var k = 0;k<this.memberInfo.length;k++){
            ...
            //  console.log('value值在这111',this.memberInfo[k])
                ...
                // else{
                //   if(this.memberInfo[k].inputClassDictCode == 'D_FEATURE_WRITETYPE_SELECT'){
                //      console.log('选择框',this.memberInfo[k]);
                //   }else{
                //     v_data.values = this.memberInfo[k].values
                //   }
                // }
          ...
          // console.log("this.memberInfo111",Message)

          let UUData ={
              "usrName": this.$route.params.liangti.usrName,
              "features": Message
          }
          // console.log("all below",this.aiId)
          
          // go 第二步
          // console.log("111object还没进来",UUData)
          // console.log("object还没进来",this.$route.params.liangti)
          if(!!this.$route.params.liangti){
            UUData.features = UUData.features.concat(this.$route.params.liangti.features)
          // Object.assign(UUData, this.$route.params.liangti);
          this.$set(UUData, "id",this.aiId);
          // console.log("object有进来",UUData)
          }
          // actionType 直接提交
          // console.log("actionType 直接提交",this.actionType)
          ...
          // 新增
          // console.log("新增",this.actionPage)
          // console.log("新增",this.aiId)
          if(this.actionPage = "newAdd" && !this.aiId){
            // console.log("0000")
              var cData = {
          ...
          // console.log("修改",UUData)
          UserService.updateAiMemberBodyInfo(UUData).then((res) => {
            //  修改会员信息
            if (res && res !== '') {
              this.$router.push({
                name:'aiCountBody',
    ...
    getMemberInfo() {
      ...
      // console.log("this.$route.params.aiId",this.$route.params)
...
//                        res[key].values = this.$route.params.data[i].valueIds
                        ...
                        if(!!this.$route.params.data[i].valueIds){
                           ...
                           if(res[key].unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_ONE'){
                              ...
                              // console.log("valuessd",res[key].values)
                           ...
                           if(res[key].unfoldClassDictCode == 'D_FEATURE_SHOWTYPE_MORE'){
                            ...
                            //  console.log("more",this.$route.params.data[i])
                                 ...
                                 for(var n = 0 ; n <res[key].valueDtoList.length;n++){
                                   if(Arr.length > 0){
                                      for(var z = 0 ; z<Arr.length;z++){
                                        // console.log("du",Arr[z])
                                        // console.log("hong",res[key].valueDtoList[n].id )
                                      ...
                                      // console.log("yuhong",res[key].valueDtoList)
                                   }
                                 }
                        ...
                        // console.log("oooo",res[key].valueDtoList)
                     }else{
            ...
            // console.log("有筛选出来",_data)
            this.memberInfo = _data;
          return
        }
        for (let key in res){
            // console.log("有item哟",res[key])
            // if(res[key].selectClassDictCode === 'D_FEATURE_CHOTYPE_OPEN'&& res[key].showPicFlag === 1){
            //   this.haveTwo = true
            // }else{
            //   this.$set(res[key], "values",'');
            // }
    ...
    //  初始化存储页面带参
      setPageParams() {
          if(!!this.$route.query) {
              ...
              if(canSetData) {
                  let bodyParams = this.$store.state.aiMeasureBody.bodyParams
                  bodyParams.query = this.$route.query
                  this.$store.commit('aiMeasureBody/setData',{
                      storeKey: 'bodyParams',
                      storeValue: bodyParams
                  })
              }
          }
          if(!!this.$route.params) {
              ...
              if(!!canSetData) {
                  let bodyParams = this.$store.state.aiMeasureBody.bodyParams
                  bodyParams.params = this.$route.params
                  this.$store.commit('aiMeasureBody/setData',{
                      storeKey: 'bodyParams',
                      storeValue: bodyParams
                  })
      ...
      setFormValue() {
          //  获取表单信息
          let memberInfo = this.$store.state.aiMeasureBody.bodyFormData
          if(!memberInfo || memberInfo.length < 0) {
              //  初始化填充仓库中的数据
              this.$store.commit('aiMeasureBody/setData', {
                  storeKey: 'bodyFormData',
  ...
  created() {
    ...
    // console.log("啦啦啦啦",this.$route.params.fromType)
    if (this.$route.params.fromType == 'goodOrder') {
