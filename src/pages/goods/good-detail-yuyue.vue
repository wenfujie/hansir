    ...
    export default {
        ...
        methods: {
            ...
            checkoutStock() {
                                ...
                                if(this.buyType == 1 || this.buyType == 3){
                                    ...
                                    for(let i=0;i<this.ms_tabList.length;i++){
                                        this.ms_getUseablePart(this.ms_tabList[i],this.goodDetail.ptiPartHdId).then((res)=>{
                                            if(res && res.length){
                                                let successUrl = '/goods-embroider?goodsCode=' + this.goodDetail.goodCode;
                                                if(this.useablePart.length){
                                                    // 已经跳转过  不用在跳转
                                                }else{
                                                    this.useablePart = res;
                                                    // unique_needCache=1在路由控制是否缓存
                                                    console.log(res,"adsdsa")

                                                    this.$router.push(`/reserve?goodCodes=${this.goodDetail.goodCode}&successUrl=${encodeURIComponent(successUrl)}&amount=${this.amount}&unique_needCache=1`)
                                                }
                                            }else{
                ...
                // 判断是否下架
                this.ms_checkoutGoodSell('isBuy').then(()=>{
                    if(!this.ms_isUnSell){
                        this.checkoutUserInfo(callback);
                    }
                })
...
//                this.$router.push("/order-settlement?orderHdId=" + res);
                    ...
                    let successUrl = "/order-settlement?orderHdId=" + res;
                    let goodCode = '';
                    if(this.buyType == 1||this.buyType == 3){
                        goodCode = this.goodDetail.goodCode;
                    }else{
                        this.groupGoodDetail.goodsList.forEach(item=>{
                            goodCode += (goodCode?',':'') + item.goodCode;
                        });
                    }
//                    Storage.set('toReserveParams',{goodCodes: goodCode,successUrl: encodeURIComponent(successUrl)});
                    // unique_needCache=1在路由控制是否缓存
                    this.$router.push(`/reserve?goodCodes=${goodCode}&successUrl=${encodeURIComponent(successUrl)}&unique_needCache=1`)
...
//                    let firstImgInfo = this.goodDetail.urlList[0];//首张图片信息
            ...
            initPage(goodsType = 'singleGood') {
                ...
                this.ms_goodsType = goodsType;//默认定制单品'singleGood'; 组合商品'groupGoods'
