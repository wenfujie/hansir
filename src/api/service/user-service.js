...
export const sellCardData = (config) => {return http.put('measure-master/sellCardData', Object.assign(config, {isMeasure: true})); }

export const getCardData = (config) => {return http.get('measure-master/getCardData', Object.assign(config, {isMeasure: true})); }
/**
 * 新增/修改会员地址
 * @param config
 * var param = {
 ...
 * @returns {*}
 */
export const getAiMemberInfo = (config) => {
   return http.get('user/aiMemberInfo', config);
}
...
//新增修改着装人数据
export const updateAiMemberBodyInfo = (config) => {
   return http.post('user/updateAiMemberBodyInfo', config);
}
...
export const getMinIntegral = (config) => {
    return http.get('user/minIntegral', config);
}