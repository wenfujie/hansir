...
function bubbleSort(arr) {
    ...
    for (var i = 0; i < len; i++) {
        for (var j = 0; j < len - 1 - i; j++) {
            if (arr[j].nodeCode > arr[j + 1].nodeCode) {
                //相邻元素两两对比
...
// 统一所有包裹配送状态
function unitPkgNodeState(packages) {
    // 取出所有的商品的订单时间
    let nodeStates = [];
    packages.forEach(pkg => {
        let nodeState = {};
        if (!check.isEmpty(pkg.nodeTime) && !check.isEmpty(pkg.nodeCode)) {
            nodeState.nodeCode = pkg.nodeCode;
            nodeState.nodeTime = pkg.nodeTime;
            nodeStates.push(nodeState);
        }
    });

    if (!check.isEmpty(nodeStates)) {
        console.log("nodeStates", nodeStates);
        //取出最小时间
        let arr = bubbleSort(nodeStates);
        let node = arr[0];
        console.log("nodeMin", node);
        // 赋值给所有包裹
        packages.forEach(pkg => {
            pkg.nodeCode = node.nodeCode;
            pkg.nodeTime = node.nodeTime;
        });
    }
    return packages;
}
...
class orderService {
    ...
    static async getList(ctx, next) {
        ...
        let res = await order.getList(ctx, params);
        console.log("res", res);
        ctx.body = {
            status: res.status,
