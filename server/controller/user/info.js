...
class info {
    ...
    //获取AI量体特征设置
    static async aiMemberInfo(ctx, next) {
        let res = await userInfo.getAiMemberInfo(ctx, ctx.params)
        ctx.body = res.data
    }
    ...
    static async updateMemberBodyInfo(ctx, next) {
     ...
     //  修改ai量体信息
     static async updateAiMemberBodyInfo(ctx, next) {
        let res = await userInfo.updateAiMemberBodyInfo(ctx, ctx.params)
        ctx.body = res
    }
    ...
    static async getMinIntegral(ctx, next) {
        let res = await userInfo.getMinIntegralValue(ctx, ctx.params)
        ctx.body = res.data
    }
